...
## Round UI-MASTER-04 (autonomo)

- [x] FASE 0 - Lettura `clientgamehud.py` (init, _update_tooltip, _draw_snapshot, _draw_activity_panel, set_map_hover, _collect_activity), `clientgame.py` (dispatch MOUSEMOTION), test esistenti `test_ui_master_03.py`, `test_hud_layout.py`, EN/IT/FR/PT-BR style.txt. Confermato baseline 421 passed / 1 skipped.
- [x] T9-CANCEL - `_collect_activity` ora restituisce 4-tupla `(kind, name, pct, unit)`. `_draw_activity_panel` registra `_activity_row_units[idx] = unit` (con unpacking permissivo per compat 3-tupla). `handle_mouse_event` intercetta click su `activity_row_N` e invoca `_cancel_unit_order(unit)` (helper best-effort che fa `orders.pop(0)`). Tooltip riga arricchito con hint via template `tooltip_activity_row_full` + chiave `tooltip_activity_cancel_hint` (EN: `Click to cancel`, IT: `Clicca per annullare`).
- [x] T9-TOOLTIP-GLOBAL - Tooltip su ogni widget HUD interattivo o informativo. Registrati rect: `res_cell_N` (loop RES bar), `food_cell` (dopo RES), `group_row_N` (loop unità del gruppo), `bottom_time` e `bottom_speed` (celle BOTTOM BAR). `_draw_snapshot` memorizza `_last_snapshot` per consentire al tooltip di leggere i valori correnti. `_update_tooltip` aggiunge branch per ogni rect, tutti localizzati via nuove chiavi (`tooltip_res_cell`, `tooltip_food_cell`, `tooltip_player_panel`, `tooltip_unit_row`, `tooltip_unit_row_nohp`, `tooltip_bottom_time`, `tooltip_bottom_speed`). `set_map_hover(None, pos, square=...)` sorgente il tooltip coords-only `tooltip_map_empty_cell` per le celle vuote della mappa.
- [x] T9-L10N-AUDIT - 10 nuove chiavi aggiunte alla sezione `def hud` di EN+IT. `res/ui-fr/style.txt` e `res/ui-pt-BR/style.txt` ristrutturati con header `def hud` (assente in precedenza) e placeholder EN marcati `; TODO: tradurre - UI-MASTER-04`. Test generativo `test_all_hud_text_keys_present_in_en_and_it` estrae via regex tutte le chiavi `_hud_(text|format|named_format)` da `clientgamehud.py` e verifica copertura completa EN+IT.
- [x] FASE-R - Revisione globale: REV-1 zero stringhe hardcoded (verificate via test generativo); REV-2 ogni nuova chiave ha default non vuoto significativo a livello di chiamata Python (LEGGE-4); REV-3 EN+IT completi su tutte le chiavi nuove e preesistenti (test generativo); REV-4 ogni elemento interattivo/informativo HUD ha tooltip (inventario: ACTIVITY header/righe/tab, EVENTS header/righe, BOTTOM BAR time/speed, RES bar celle, FOOD cella, PLAYER, GROUP righe, MAP entità, MAP cella vuota); REV-5 layout 800x600 e 1024x768 sani (test_hud_layout esistenti restano verdi); REV-6 difensività (`_cancel_unit_order` swallow + try/except, branch tooltip controllano `self._last_snapshot is not None`); REV-7 nessuna API inventata (tutti i metodi nuovi sono privati e definiti localmente).
- [x] FASE-T - 21 test in `test_ui_master_04.py` PASS (5 T9-CANCEL + 13 T9-TOOLTIP-GLOBAL + 3 T9-L10N-AUDIT). Suite completa 442 passed / 1 skipped (baseline +21, zero regressioni).
- [x] FASE-D - CHANGELOG e TODO aggiornati. Valutazione release: criteri (a) funzionalità autonome significative SI', (b) zero regressioni SI', (c) docs sincronizzate SI', (d) EN+IT completi SI'. Proposta bump versione patch (es. da 1.2.4 a 1.2.5) opportuna ma non eseguita. Nessun comando git eseguito.
