...
- [UI-SIGHTED-03/SI-10] `soundrts/lib/mouse.py`: nuovo cursore custom `sizeall` (pattern 8×8 con 4 frecce diagonali, hotspot 4,4) registrato via `record_cursor`. Evocativo di "muovi in ogni direzione", visivamente distinto da `attack` (angoli pieni) e `target` (reticolo centrato). `soundrts/clientgame.py`: nuovo metodo `GameInterface._apply_rubber_band_cursor(pos)` che durante il drag di selezione (`mouse_select_origin` popolato e diverso da `pos`) sostituisce il cursore con `sizeall`. Iniettato in cima al ramo MOUSEMOTION di `_process_fullscreen_mode_mouse_event` con `getattr` difensivo (compat test SimpleNamespace); ha priorità sui cursori SI-07 (`attack`/`diamond`). Guardia LEGGE-6 su `display_is_active`.

### Changed

- [UI-SIGHTED-03/SI-09] `soundrts/clientgamegridview.py`: `display_objects()` rifattorizzato per aggregare le unità per cella E per categoria proprietario (`own`/`enemy`/`ally`). Classificazione: `obj.player is interface.player` → `own`; altrimenti `player_is_an_enemy(own_player)` → `enemy`; altrimenti → `ally`. Eccezione su `player_is_an_enemy` ricade su `ally` (safe-by-default). Complessità invariata O(N) — costante K=3 op/oggetto, nessun ciclo annidato. Badge dispatchati ai 3 anchor con colori dedicati: verde `(60,166,60)` proprio sopra-destra, rosso `(166,60,60)` nemico sopra-sinistra, grigio `(100,100,120)` alleato sotto-destra.
- [UI-SIGHTED-03/SI-09] `soundrts/clientgamegridview.py`: `_draw_stack_badge()` esteso con parametri opzionali `color: Tuple[int,int,int] = (200,200,100)` e `border_color: Optional[...]`. Retrocompatibile (firma posizionale invariata). Default risultano nella stessa palette giallo-pastello SI-05 quando non specificati. La label è ora `(240,240,240)` per contrasto uniforme su qualunque tinta. Il caller passa la posizione FINALE del badge (non l'ancora dell'unità): l'offset `+6/-16` è stato spostato in `display_objects()` per consentire posizionamenti differenti per categoria.

### Fixed

- [UI-SIGHTED-03/SI-08] `soundrts/clientgamehud.py`: pannello KEYS ora alimentato da `res/ui/bindings.txt` invece della lista hardcode SI-06. Nuovo metodo statico `HudPanel._load_bindings_from_file(path)` (parser robusto: skip righe vuote/commenti `;`/`#`, gestione `KEY[ MOD ...]: action [args]`, prima associazione vince, malformed → log stderr `[SOUNDRTS-VISUAL][WARNING]`, file mancante → `{}`). Lazy-load con cache via `_get_bindings()` (`_bindings_cache: Optional[Dict[str,str]]` inizializzato a `None` in `__init__`, O(1) dopo primo accesso). `_KEYS_HOTKEYS` esteso da 3-tuple a 4-tuple (aggiunto `bindings_action_name`); 8 nuove voci mappate a azioni reali SoundRTS (`toggle_pause`, `unit_status`, `default`, `do_again`, `immersion`, `examine`, `select_square`, `select_scouted_square`) sostituiscono le scorciatoie RTS canoniche (S/A/P) che non erano effettivamente bound. `_draw_keys_panel` ora cerca la chiave reale via `bindings.get(action_name) or key_label_default`. Larghezza pannello aumentata 240→280 px e formato label da `{:<8}` a `{:<12}` per accogliere le hotkey lunghe (PAGEUP/DN, CTRL+SPACE).
- [UI-SIGHTED-03/SI-08/L10N] `res/ui/style.txt` e `res/ui-it/style.txt`: aggiunte 8 nuove chiavi L10N per le label del pannello KEYS (`hotkey_pause`, `hotkey_status`, `hotkey_default`, `hotkey_again`, `hotkey_immersion`, `hotkey_examine`, `hotkey_arrows`, `hotkey_scout`) in EN e IT (LEGGE-3). Le chiavi SI-06 precedenti (`hotkey_space/f/s/a/p/tab/esc/ctrl_a`) restano disponibili come dichiarate, anche se non più referenziate dal nuovo `_KEYS_HOTKEYS` (non rimosse per evitare regressioni in caso di mod che le usano).

### Tests

- [UI-SIGHTED-03] `soundrts/tests/unittests/test_ui_sighted_03.py`: 15 nuovi test (5 SI-08: parsing bindings.txt reale, path inesistente, skip commenti, cache `is`, fallback monkeypatch; 4 SI-09: colore verde verificato pixel-perfect, colore rosso verificato pixel-perfect, retrocompat firma legacy, aggregazione owner via `display_objects` mock; 6 SI-10: cursore registrato, attivazione drag, no-origin/display-off/zero-drag guards, priorità su attack). LEGGE-10 rispettata (zero import cross-test).
- [UI-SIGHTED-03] `soundrts/tests/unittests/test_ui_sighted_02.py`: aggiornato `test_keys_panel_has_eight_hotkey_entries` per il nuovo schema 4-tuple di `_KEYS_HOTKEYS` (LEGGE-7: aggiornato, mai cancellato). Verifica forward-compatibile (unpack dei primi 3 campi).

Suite finale: **515 passed, 1 skipped** (baseline UI-SIGHTED-02: 500/1; +15 test, 0 regressioni).

## [Pre-UI-SIGHTED-03 baseline]

### Added

- [UI-SIGHTED-02/SI-03b] `soundrts/clientgamegridview.py`: nuovo metodo `GridView.draw_rubber_band(screen, start, end)` che disegna un overlay verde semi-trasparente `(100, 200, 100, 180)` width=1 su una `Surface(SRCALPHA)` riallocata per frame. Skip per rettangoli degenerati (`w<2` o `h<2`). LEGGE-4: ogni eccezione è loggata su `stderr` con prefisso `[SOUNDRTS-VISUAL][ERROR] draw_rubber_band:` e mai propagata. `soundrts/clientgame.py`: il blocco rect bianco precedente in `display()` (linee ~2257-2265) è stato sostituito da chiamata centralizzata al nuovo helper, attiva solo quando `mouse_select_origin != mouse.get_pos()` e racchiusa in try/except difensivo.
- [UI-SIGHTED-02/SI-05] `soundrts/clientgamegridview.py`: nuovo metodo `GridView._draw_stack_badge(screen, pos, count)` che disegna un badge 18×14 px (fill `(30,30,30,210)` + bordo `(200,200,100)` + label centrata `(230,230,140)`) sopra-destra dell'ancora dell'unità. Mostra `"99+"` per count > 99, skip per count ≤ 1. `display_objects()` rifattorizzato per aggregare `id(place)` durante il loop di rendering esistente (zero passi aggiuntivi: O(N) invariato) e disegnare i badge SOLO per celle con > 1 unità. Indicatore visivo immediato per stack di unità — risolve GAP-06 del report sighted.
- [UI-SIGHTED-02/SI-06] `soundrts/clientgamehud.py`: nuovo pannello HUD collassabile `KEYS` con leggenda hotkey. Nuovo flag `HudPanel.keys_visible: bool = False` (collassato per default, layout esistente invariato), nuovo metodo `_draw_keys_panel(screen, left, bottom_bar_top)` che disegna il pannello in basso a sinistra sopra la bottom bar. Lista hotkey via attributo classe `_KEYS_HOTKEYS` (8 voci: SPACE, F, S, A, P, TAB, ESC, CTRL+A). Header cliccabile via `_panel_rects["keys_header"]` con toggle in `handle_mouse_event`. Tutte le label via `_hud_text(l10n_key, default_en)` con fallback inglese (LEGGE-4). Risolve GAP-07 del report sighted.
- [UI-SIGHTED-02/SI-07] `soundrts/lib/mouse.py`: nuovo cursore custom `attack` (crosshair 8×8 con croce X ai vertici e cerchio centrale) registrato via `record_cursor("attack", (4, 4), ...)`. `set_cursor(name)` reso difensivo: cursori sconosciuti o `pygame.mouse.set_cursor` falliti sono ignorati silenziosamente (mai crash dell'input loop). `soundrts/clientgamegridview.py`: nuovo metodo `GridView.enemy_at_mousepos(pos)` che ritorna l'entità sotto il cursore SOLO se appartiene a un giocatore nemico (`obj_player.player_is_an_enemy(own_player)`), `None` per celle vuote / unità proprie / alleati / errori (try/except totale, LEGGE-5). `soundrts/clientgame.py`: nel handler MOUSEMOTION, quando NON è attivo un ordine target-required, il cursore commuta a `attack` su hover di entità nemica, `diamond` altrimenti.

### Fixed

- [UI-SIGHTED-02/SI-L10N] `res/ui/style.txt`: aggiunte 15 chiavi `ctx_*` mancanti nella base inglese (`ctx_move`, `ctx_attack`, `ctx_stop`, `ctx_patrol`, `ctx_build`, `ctx_train`, `ctx_research`, `ctx_repair`, `ctx_use`, `ctx_default`, `ctx_cancel_training`, `ctx_cancel_upgrading`, `ctx_cancel_building`, `ctx_join_group`, `ctx_reset_group`) e 12 chiavi pannello KEYS (`panel_keys`, `keys_collapsed`, `tooltip_keys_show`, `tooltip_keys_hide`, `hotkey_space`, `hotkey_f`, `hotkey_s`, `hotkey_a`, `hotkey_p`, `hotkey_tab`, `hotkey_esc`, `hotkey_ctrl_a`). Le chiavi `ctx_*` erano state aggiunte solo in `res/ui-it/style.txt` durante UI-SIGHTED-01, lasciando i giocatori non-IT con label vuote nel context menu. `res/ui-it/style.txt`: aggiunte le 12 chiavi `panel_keys/hotkey_*` per il pannello KEYS in italiano.

### Tests

- [UI-SIGHTED-02] `soundrts/tests/unittests/test_ui_sighted_02.py`: 14 nuovi test (2 SI-L10N: chiavi ctx_*/hotkey_* presenti in EN e IT; 3 SI-03b: skip su rettangoli minimali, blit valido, no propagazione eccezioni; 3 SI-05: skip count ≤ 1, blit per count > 1, "99+" per count ≥ 100; 3 SI-06: default `keys_visible=False`, toggle via click header, struttura `_KEYS_HOTKEYS` valida; 3 SI-07: enemy detection / own unit / cursore custom registrato in `lib.mouse`). Suite totale: **487 → 500 passed, 1 skipped**.

## [Pre-UI-SIGHTED-02 baseline]

### Added

- [UI-SIGHTED-01/SI-01] `soundrts/clientgamegridview.py`: nuovo metodo `GridView.entity_at_mousepos(pos)` che restituisce l'oggetto sotto al cursore SOLO se appartiene a `interface.player`; ritorna `None` per celle vuote, unità nemiche/alleate o qualsiasi errore (try/except difensivo, LEGGE-5). Riusa `object_from_mousepos` senza duplicare la logica di hit-test.
- [UI-SIGHTED-01/SI-01] `soundrts/clientgamehud.py`: aggiunte dataclass `ContextMenuItem`, `ContextMenu`, `OrderFlash` e nuova famiglia di metodi su `HudPanel` — `show_context_menu(unit, pos)`, `hide_context_menu()`, `_draw_context_menu(screen)`, `_ctx_item_at(menu, pos)`, `_context_menu_rect(menu)`, `handle_context_menu_event(event)`, `_execute_context_menu_item(item, pos)` — che traducono il right-click su un'unità propria in una visualizzazione testuale del menu ordini canonico letto da `interface.orders()` (LEGGE-7: nessuna divergenza dalla pipeline audio). Le voci con `nb_args == 0` vengono dispatchate via `interface.send_order(keyword, None, [])`; quelle con target richiedono il successivo left-click via `interface._select_order(view)`. Hook in `handle_mouse_event` (consuma eventi quando il menu è aperto) e in `display()` (disegno post-`_draw_snapshot`, auto-close in pausa). Logging strutturato su `sys.stderr` con prefisso `[SOUNDRTS-VISUAL][ERROR]` per ogni fallimento del layer visivo. Helper `_visual_log_error(msg)` modulare.
- [UI-SIGHTED-01/SI-01] `soundrts/clientgame.py`: nel branch `e.button == 3` di `_process_fullscreen_mode_mouse_event`, sondaggio `grid_view.entity_at_mousepos(e.pos)` PRIMA della chiamata a `cmd_default`. Se trova un'unità propria e il gioco non è in pausa, apre il menu via `hud_panel.show_context_menu(entity, e.pos)` e ritorna early; altrimenti preserva il comportamento legacy (movimento/attacco + `flash_move_target`).
- [UI-SIGHTED-01/SI-04] `soundrts/clientgamehud.py`: feedback visivo transiente per ordini non-movimento dispatchati via context menu. Nuovo dict `_order_flashes: Dict[str, OrderFlash]`, costanti `_ORDER_FLASH_COLORS` (attack rosso `(200,80,80)`, stop grigio `(220,220,220)`, patrol giallo `(220,200,80)`) e `_ORDER_FLASH_DURATION = 0.4s`. Metodi `flash_order(keyword, pos)` e `_draw_order_flashes(screen)` con decadimento temporale via `time.monotonic()` e pulizia automatica delle entry scadute. Disegnati DOPO `_draw_move_flash` e PRIMA del menu, così non si sovrappongono al pannello modale.
- [UI-SIGHTED-01/L10N] `res/ui-it/style.txt`: nuove chiavi `[hud]` per le label del context menu (`ctx_move Muovi`, `ctx_attack Attacca`, `ctx_stop Ferma`, `ctx_patrol Pattuglia`, `ctx_build Costruisci`, `ctx_train Addestra`, `ctx_research Ricerca`, `ctx_repair Ripara`, `ctx_use Usa`, `ctx_default Predefinito`, `ctx_cancel_training`, `ctx_cancel_upgrading`, `ctx_cancel_building`, `ctx_join_group`, `ctx_reset_group`). Fallback inglese hardcoded in `HudPanel._CTX_LABEL_FALLBACK` (LEGGE-4). Lookup tramite `style.get("hud", "ctx_" + keyword)` direttamente (evita la regex T9-L10N-AUDIT su chiavi dinamiche; LEGGE-4 rispettata).
- [UI-SIGHTED-01] `soundrts/tests/unittests/test_ui_sighted_01.py`: 13 nuovi test (8 SI-01: show no-op su `None`, popolamento items da `interface.orders()`, no-open in pausa, ESC chiude, click su voce no-target dispatcha `send_order` + flash, click fuori chiude senza dispatch, voce target-required invoca `_select_order`; 3 SI-04: registrazione flash per keyword nota, ignorato per keyword senza colore, pruning entry scadute; 3 GridView: own/foreign/empty su `entity_at_mousepos`). Pattern lazy-import + `MagicMock` + `SimpleNamespace`.
- [UI-SIGHTED-01] `doc_admin/report_sighted_ux_01.md`: report MACRO-1 con inventario funzionalità visive, gap analysis (GAP-01..08), roadmap implementativa, architettura context menu, criteri di accettazione. Documenta SI-02 (selezione left-click) e SI-03 base (rubber-band) come `ALREADY-DONE` con riferimento al sorgente esistente; rimanda outline drag, minimap, mini-portrait e UI ordini per riga unità a UI-SIGHTED-02.
