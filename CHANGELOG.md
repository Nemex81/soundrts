...
- [UI-MASTER-01/T3] `soundrts/clientgamehud.py`, `soundrts/clientgame.py`: pannello EVENTS collassabile. Nuovo attributo `HudPanel.events_visible` (default True), nuovo metodo `cmd_toggle_events()` su `GameInterface` con voicing TTS, hit-test sul rect `events_header` in `HudPanel.handle_mouse_event()`. Header mostra il suffisso `(hidden)` quando collassato.
...
- [UI-MASTER-01] `res/ui/bindings.txt`: binding `ALT e` → `toggle_events`, `ALT a` → `toggle_activity_panel`, `ALT 1/2/3/4` → tab selettori del pannello attività.
